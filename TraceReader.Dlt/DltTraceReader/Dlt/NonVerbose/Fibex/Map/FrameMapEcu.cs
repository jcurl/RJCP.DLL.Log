namespace RJCP.Diagnostics.Log.Dlt.NonVerbose.Fibex.Map
{
    ...
    /// </summary>
    /// <remarks>
    /// Similar to <see cref="FrameMapDefault"/>, if no ECU is added, then this becomes the same as
    /// <see cref="FrameMapDefault"/>. Adding to this map always adds the frame for all ECUs (and if there is another
    ...
    /// <para>
    /// Thus, behaviour extends the DLT Viewer implementation, but if the FIBEX file is missing the ECU, or the log line
    /// doesn't have the ECUID in the standard header, the behaviour is the same as the DLT Viewer.
    /// </para>
    /// </remarks>
    internal class FrameMapEcu : IFrameMapLoader
    {
        private readonly FrameMapDefault m_Frames = new FrameMapDefault();
        private readonly Dictionary<string, FrameMapDefault> m_EcuFrames = new Dictionary<string, FrameMapDefault>();
        ...
        public bool TryAddFrame(int id, string appId, string ctxId, string ecuId, IFrame frame)
        {
            ...
            if (ecuId is object) {
                ...
                m_EcuFrames.Add(ecuId, new FrameMapDefault(id, appId, ctxId, frame));
                return true;
            }
            return success;
        }
