    ...
    public sealed class NonVerboseUnsignedIntArgDecoder : INonVerboseArgDecoder
    {
        ...
        public int Decode(ReadOnlySpan<byte> buffer, bool msbf, IPdu pdu, out IDltArg arg)
        {
            if (buffer.Length < pdu.PduLength)
                return DltArgError.Get($"Insufficient payload buffer {pdu.PduLength} for unsigned int argument", out arg);
            if (pdu.PduLength < m_IntLength)
                return DltArgError.Get($"S_UINT{m_IntLength * 8} invalid length in PDU", out arg);

            if (m_IntLength == 1) {
                arg = new UnsignedIntDltArg(buffer[0]);
            } else if (m_IntLength == 2) {
                arg = new UnsignedIntDltArg(unchecked((ushort)BitOperations.To16Shift(buffer, !msbf)));
            } else if (m_IntLength == 4) {
                arg = new UnsignedIntDltArg(unchecked((uint)BitOperations.To32Shift(buffer, !msbf)));
            } else if (m_IntLength == 8) {
                arg = new UnsignedIntDltArg(unchecked((ulong)BitOperations.To64Shift(buffer, !msbf)));
            } else {
                throw new InvalidOperationException("Unexpected data length");
            }
