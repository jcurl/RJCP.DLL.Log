namespace RJCP.Diagnostics.Log.Dlt.Control
{
    ...
    /// </summary>
    public sealed class SetMessageFilteringRequestDecoder : IControlArgDecoder
    {
        /// <summary>
        ...
        public Result<int> Decode(int serviceId, ReadOnlySpan<byte> buffer, bool msbf, out IControlArg service)
        {
            if (buffer.Length < 5) {
                service = null;
                return Result.FromException<int>(new DltDecodeException($"'SetMessageFilteringRequest' with insufficient buffer length of {buffer.Length}"));
            }
