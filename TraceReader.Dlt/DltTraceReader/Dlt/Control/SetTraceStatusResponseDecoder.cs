namespace RJCP.Diagnostics.Log.Dlt.Control
{
    ...
    /// </summary>
    public sealed class SetTraceStatusResponseDecoder : IControlArgDecoder
    {
        /// <summary>
        ...
        public Result<int> Decode(int serviceId, ReadOnlySpan<byte> buffer, bool msbf, out IControlArg service)
        {
            if (buffer.Length < 5) {
                service = null;
                return Result.FromException<int>(new DltDecodeException($"'SetTraceStatusResponse' with insufficient buffer length of {buffer.Length}"));
            }

            int status = buffer[4];
            if (status == ControlResponse.StatusError ||
                status == ControlResponse.StatusNotSupported) {
                service = new ControlErrorNotSupported(serviceId, status, "set_trace_status");
                return 5;
            }
