namespace RJCP.Diagnostics.Log.Dlt.Control
{
    ...
    /// </summary>
    public sealed class SetTraceStatusRequestDecoder : IControlArgDecoder
    {
        /// <summary>
        ...
        public Result<int> Decode(int serviceId, ReadOnlySpan<byte> buffer, bool msbf, out IControlArg service)
        {
            if (buffer.Length < 17) {
                service = null;
                return Result.FromException<int>(new DltDecodeException($"'SetTraceStatusRequest' with insufficient buffer length of {buffer.Length}"));
            }
