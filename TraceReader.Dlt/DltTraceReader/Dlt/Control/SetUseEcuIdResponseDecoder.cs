namespace RJCP.Diagnostics.Log.Dlt.Control
{
    ...
    /// <summary>
    /// Decodes the contents of the buffer to return a <see cref="SetUseEcuIdResponse"/>.
    /// </summary>
    public sealed class SetUseEcuIdResponseDecoder : IControlArgDecoder
    {
        /// <summary>
        ...
        public Result<int> Decode(int serviceId, ReadOnlySpan<byte> buffer, bool msbf, out IControlArg service)
        {
            if (buffer.Length < 5) {
                service = null;
                return Result.FromException<int>(new DltDecodeException($"'SetUseEcuIdResponse' with insufficient buffer length of {buffer.Length}"));
            }
