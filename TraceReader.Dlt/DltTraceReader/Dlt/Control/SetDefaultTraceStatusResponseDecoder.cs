namespace RJCP.Diagnostics.Log.Dlt.Control
{
    ...
    /// </summary>
    public sealed class SetDefaultTraceStatusResponseDecoder : ControlArgDecoderBase
    {
        /// <summary>
        ...
        public override int Decode(int serviceId, ReadOnlySpan<byte> buffer, bool msbf, out IControlArg service)
        {
            if (buffer.Length < 5)
                return DecodeError(serviceId, DltType.CONTROL_RESPONSE,
                    "'SetDefaultTraceStatusResponse' with insufficient buffer length of {0}", buffer.Length,
                    out service);
