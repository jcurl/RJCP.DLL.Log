namespace RJCP.Diagnostics.Log.Dlt.ControlEncoder
{
    ...
    [TestFixture(typeof(GetUseExtendedHeaderResponseEncoder), EncoderType.Argument, Endianness.Big)]
    [TestFixture(typeof(GetUseExtendedHeaderResponseEncoder), EncoderType.Argument, Endianness.Little)]
    ...
    public class GetUseExtendedHeaderResponseEncoderTest<TControlEncoder>
        : ControlEncoderTestBase<TControlEncoder> where TControlEncoder : IControlArgEncoder
    {
        public GetUseExtendedHeaderResponseEncoderTest(EncoderType encoderType, Endianness endianness)
            : base(encoderType, endianness) { }

        [Test]
        public void EncodeSuccess([Values(false, true)] bool enabled)
        {
            GetUseExtendedHeaderResponse response =
                new GetUseExtendedHeaderResponse(ControlResponse.StatusOk, enabled);

            Span<byte> buffer = ControlEncode(response, 6);
            Assert.That(BitOperations.To32Shift(buffer[0..4], !IsBigEndian), Is.EqualTo(0x1E));
        ...
        {
            GetUseExtendedHeaderResponse response =
                new GetUseExtendedHeaderResponse(statusCode, false);
        ...
        public void InsufficientBuffer([Range(3, 5, 1)] int length)
        {
            ...
            GetUseExtendedHeaderResponse response =
                new GetUseExtendedHeaderResponse(ControlResponse.StatusOk, true);
