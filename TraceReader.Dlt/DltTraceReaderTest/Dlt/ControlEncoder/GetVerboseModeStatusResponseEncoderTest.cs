namespace RJCP.Diagnostics.Log.Dlt.ControlEncoder
{
    ...
    [TestFixture(typeof(GetVerboseModeStatusResponseEncoder), EncoderType.Argument, Endianness.Big)]
    [TestFixture(typeof(GetVerboseModeStatusResponseEncoder), EncoderType.Argument, Endianness.Little)]
    ...
    public class GetVerboseModeStatusResponseEncoderTest<TControlEncoder>
        : ControlEncoderTestBase<TControlEncoder> where TControlEncoder : IControlArgEncoder
    {
        public GetVerboseModeStatusResponseEncoderTest(EncoderType encoderType, Endianness endianness)
            : base(encoderType, endianness) { }

        [Test]
        public void EncodeSuccess([Values(false, true)] bool enabled)
        {
            GetVerboseModeStatusResponse response =
                new GetVerboseModeStatusResponse(ControlResponse.StatusOk, enabled);

            Span<byte> buffer = ControlEncode(response, 6);
            Assert.That(BitOperations.To32Shift(buffer[0..4], !IsBigEndian), Is.EqualTo(0x19));
        ...
        {
            GetVerboseModeStatusResponse response =
                new GetVerboseModeStatusResponse(statusCode, false);
        ...
        {
            GetVerboseModeStatusResponse response =
                new GetVerboseModeStatusResponse(ControlResponse.StatusOk, true);
